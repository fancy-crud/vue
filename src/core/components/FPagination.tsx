...
export function FPagination(props: FPaginationAttributes) {
  ...
  const onSelectPage = (page: number) => {
    if (!pages.length || page < 1 || page > pager.totalPages) return
  ...
  if (!pager.pages.includes(1)) {
    pages.push(
      (
        <button onClick={() => onSelectPage(1)} class="btn btn-ghost">
          1
        </button>
      ) as HTMLButtonElement
    )
    pages.push((<button class="btn btn-ghost btn-disabled">...</button>) as HTMLButtonElement)
  }

  pager.pages.forEach((p) => {
    const _class = pagination.page === p ? "btn btn-active" : "btn btn-ghost"
    const page = (
      <button onClick={() => onSelectPage(p)} class={_class}>
        {p}
      </button>
    ) as HTMLButtonElement
    pages.push(page)
  ...
  if (!pager.pages.includes(pager.totalPages)) {
    pages.push((<button class="btn btn-ghost btn-disabled">...</button>) as HTMLButtonElement)
    pages.push(
      (
        <button onClick={() => onSelectPage(pager.totalPages)} class="btn btn-ghost">
          {pager.totalPages}
        </button>
      ) as HTMLButtonElement
    )
  ...
  return (
    <div class="btn-group items-center">
      <button onClick={() => onSelectPage(pagination.page - 1)} class="btn btn-ghost">
